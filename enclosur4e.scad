...
$fn = 72;
eps = 0.01;
over = 0.5;      // выход вырезов за поверхность: совпадающие грани дают артефакты в слайсере
...
/* ================= ПЛАТЫ (справочно, для проверки коллизий) =========
   Бобышки крепежа занимают круги d=6 в углах (5.5,5.5) (66.5,5.5)
   (5.5,48.5) (66.5,48.5) на всю высоту полости — платы их обходят.

   Nano      18 x 45, x  9..27, y  4..49, USB к задней стенке
   MCP2515   28 x 40, x 27..55, y  6..46
   Mini360   11 x 15, x 55..66, y 30..45
   KY-006    15 x 18.5, x 52..67, y 10..28.5
   OLED      27.5 x 27.5, к крышке, центр (34, 28)
   TTP223    15 x 11, к крышке, центр (11, 16) — x 3.5..18.5, y 10.5..21.5
...
glue_pads = [[60.5, 32.5], [60.5, 42.5],  // Mini360
             [59.5, 13],  [59.5, 25.5]];  // KY-006
...
dimple_d     = 15.0;
dimple_depth = 1.8;
// Центр смещён влево: лунка X 3.5..18.5, карман OLED от X 20.2 — зазор 1.7.
dimple_x     = 11;     // = центр TTP223
dimple_y     = 16;     // выше бобышки (Y<=8.5): лунка Y 8.5..23.5
...
label_1 = "BMW E90";
label_2 = "CAN BUS";
// Полоса между лункой (правый край X=18.5) и отверстием LED (левый край X=42.5).
// BMW E90 — X 23.3..40.7, CAN BUS — X 25..39.
label_x = 32;
label_1_y = 12.5;
label_2_y = 6.0;
label_1_size = 4.0;
...
buzz_y = 20;
buzz_hole_d = 1.6;
buzz_rings  = [[3.4, 6], [6.1, 9], [8.8, 12]];
buzz_span   = 130;    // угловой раствор веера, градусов
// габарит дуг: x 61 +/- 8.0 = 53.0..69.0, y 11.2..20 — внутри стенок
...
vent_floor_h  = 2;
vent_floor_x  = 55;   // щели между клеевыми площадками Mini360
vent_floor_ys = [36.5, 38.5];
...
module lid_engrave(txt, size, x, y, z_top) {
  translate([x, y, z_top - engrave_d])
    linear_extrude(height = engrave_d + over)
...
module shell() {
  union() {
    difference() {
      ...
      translate([oled_cx, oled_cy + win_dy, cavity_h - over])
        linear_extrude(height = lid_t + 2 * over, scale = [
      ...
      translate([led_x, led_y, cavity_h - over])
        cylinder(h = lid_t + 2 * over, d = led_d);
