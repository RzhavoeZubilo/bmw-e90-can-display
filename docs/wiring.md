...
|---|---|
| Arduino Nano | ~20 мА |
| MCP2515 + TJA1050 (только приём) | ~10 мА |
| OLED | ~15 мА |
...
Берите +12 В с цепи, которая обесточивается вместе с зажиганием. Если такой
рядом нет — ставьте выключатель и не забывайте им пользоваться, но это
заведомо хуже.
...
### Устройство ничего не передаёт

Прошивка работает в `MCP_LISTENONLY`: физически не может отправить в моторную
шину ни одного кадра. Передаёт только диагностический сканер `sniffer`, и то
во второй фазе — стандартные запросы, которые шлёт любой сканер.
